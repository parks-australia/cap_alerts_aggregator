...
const boundariesDirectory = resolve(
  process.env.BOUNDARIES_DIR ?? "assets/boundary_data",
);
if (process.env.BOUNDARIES_DIR !== "none") {
  ...
  await Promise.all(
    Object.entries(parkOutputs).map(([parkId, parkOutput]) =>
      writeFile(
        `${outputDirectory}/${parkId}.json`,
        `${JSON.stringify(parkOutput, null, 2)}\n`,
        "utf8",
      ),
    ),
  );
  console.log(
    `Wrote ${Object.keys(parkOutputs).length} per-park files to ${outputDirectory}`,
  );
}
  ...
  return readFile(envPath, "utf8")
    ...
    .catch((error) => {
      if (error.code === "ENOENT") {
        throw new Error(
          "Missing .env; copy .env.example to .env and set the local Drupal values.",
        );
      }
      throw error;
    });
